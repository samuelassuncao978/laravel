@php
$style_bold = 'bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 pl-28';
$style_normal = 'appearance-none outline-none focus:ring-2  focus:shadow-inner shadow-sm ring-gray-300 focus:border-gray-400 block w-full bg-white text-gray-500 text-sm border border-gray-100 rounded py-3 px-4 pl-28';
...
<div {{ $attributes->only('class') }}>
    ...
    <div class="flex relative">
        ...
        <input type="text" {!! $attributes->has('wire:model') ? ' wire:model="' . $attributes->get('wire:model') . '.number"' : '' !!} {!! $attributes->has('wire:model.defer') ? ' wire:model.defer="' . $attributes->get('wire:model.defer') . '.number"' : '' !!}
            value="{{ optional($value)['number'] ?? '' }}"
            name="{{ $attributes->get('name') }}-number"
            class="w-full bg-transparent focus:outline-none py-3 px-4  pl-28"
            {{ $attributes->except(['class', 'value', 'wire:model', 'wire:model.defer']) }} />
        <span
            class="absolute flex transform -translate-y-1/2 top-1/2  {{ isset($bold) ? 'left-1.5' : 'left-3' }} flex-shrink-0 w-24">
            <select {!! $attributes->has('wire:model') ? ' wire:model="' . $attributes->get('wire:model') . '.country"' : '' !!} {!! $attributes->has('wire:model.defer') ? ' wire:model.defer="' . $attributes->get('wire:model.defer') . '.country"' : '' !!}
                {{ $attributes->except(['class', 'value', 'name', 'id', 'wire:model', 'wire:model.defer']) }}
                value="{{ optional($value)['country'] ?? '' }}"
                  name="{{ $attributes->get('name') }}-country"
                class="px-2 pr-6 py-1 appearance-none outline-none focus:shadow-inner shadow-sm  block w-full ring-gray-300 focus:border-gray-400  bg-gray-100 bg-opacity-50 border-gray-100 text-gray-500 text-xs border rounded">
                @foreach ($codes as $country => $d)
                    <option value="+{{ $d['code'] }}">({{ $country }}) +{{ $d['code'] }}</option>
                @endforeach
            </select>
