...
<x-form.label {{ $attributes }} />
    <span x-data="{value: @entangle($attributes->wire('model')) }" {{ $attributes->only('class')->class(['border-gray-300 focus-within:border-blue-400 focus-within:ring-blue-400'=> !$attributes->has('class') ])->merge(['class' => 'flex items-center justify-center relative text-sm flex-grow border-2  bg-white outline-none focus-within:ring-2 focus-within:shadow-inner focus-within:ring-opacity-20 rounded transition-all ease-in-out duration-200']) }}>
      
        <span class=" flex relative flex-shrink-0 w-24 text-gray-400 focus-within:text-white">
            <select x-model="value.country" class="ml-1.5 px-2 pr-6 py-1 appearance-none outline-none focus:shadow-inner shadow-sm  block w-full focus:bg-gray-600 focus:text-white bg-gray-300 bg-opacity-50 text-gray-500 text-xs rounded">
                @foreach ($codes as $country => $d)
                    <option value="{{ $country }}">({{ $country }}) +{{ $d['code'] }}</option>
                @endforeach
            </select>
            <svg xmlns="http://www.w3.org/2000/svg"
                class="h-3/5  top-1/2 transform -translate-y-1/2 right-1 absolute pointer-events-none"
                fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 9l4-4 4 4m0 6l-4 4-4-4" />
            </svg>
        </span>
        @if(isSet($prepend)) {{ $prepend }} @endif
            <input x-model.debounce.500ms="value.number" id="{{ collect($attributes->only(['id','name','wire:model','label']))->first() }}" {{ $attributes->except(['name','class','wire:loading','wire:target','wire:model','loading','prepend','append']) }} class="outline-none bg-transparent text-lg md:text-sm text-gray-700 w-full pl-2 px-4 p-2 flex-1" />
