        ...
        <div class="relative flex-grow justify-center flex lg:items-center bg-gray-100 p-5">
            <div class="w-full lg:w-1/2 flex lg:items-center justify-center -mt-20 lg:mt-0">
                <div class="bg-white shadow-lg flex-grow max-w-lg rounded-lg py-10 px-10 lg:px-20">
                    <div class="font-bold text-lg text-gray-700 mb-4">Reset</div>
                    <div class="relative">
                        <div class="-mx-3 flex flex-col">

                        @if(!$this->client)
                            <div>Invalid reset token</div>
                        @elseif($this->client && $this->expires->isBefore(now()))
                            <div>This reset token has expired.</div>
                        @else
                            <div>
                                <x-ui.form.field type="password" name="password" label="New password" model="password" />
                            </div>
                        @endif
                        ...
                        <div class="flex justify-between items-center mt-6">
                            <a href="/portal" class="text-gray-500 hover:blue-500 hover:underline">Back</a>
                            <span>
                                <x-ui.button.primary bold type="submit" id="Login">Reset</x-ui.button.primary>
                            </span>
                        </div>
                    </div>
                </div>
