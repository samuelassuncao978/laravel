<div class="flex flex-col flex-grow overflow-hidden h-full">
    ...
    @if ($this->on('App\Http\Livewire\Admin\Plugins\Edit'))
        @livewire('admin.plugins.edit', $this->parameters('App\Http\Livewire\Admin\Plugins\Edit'))
    @endif

    @if ($this->on('App\Http\Livewire\Admin\Plugins\Create'))
        @livewire('admin.plugins.create', $this->parameters('App\Http\Livewire\Admin\Plugins\Create'))
    @endif
    ...
    <div class="sm:px-7 -mb-0.5 px-4 relative z-10 flex items-center bg-gray-50 py-3 border-b border-gray-200">
        ...
        <div class="inline-block ml-auto">
            <x-ui.button.standard compact bold
                wire:click="{{ $this->opens('App\Http\Livewire\Admin\Plugins\Create', ['tenant' => $tenant]) }}">
                Add Plugin
                        ...
                        @slot('rows')
                            @foreach ($plugins as $key => $plugin)
                                <tr class="hover:bg-blue-50 hover:bg-opacity-50 cursor-pointer"
                                    wire:click="{{ $this->opens('App\Http\Livewire\Admin\Plugins\Edit', ['plugin' => $plugin]) }}">
            ...
            @else
                <div class="h-full flex items-center justify-center">
                    @if (empty($this->search))
                        <x-common.empty-notice title="No plugins">
                            <x-icon.outline icon="cog" />
                        </x-common.empty-notice>
                    @else
                        <x-common.empty-notice title="No plugins found">
                            <x-icon.outline icon="cog" />
                        </x-common.empty-notice>
                    @endif
                </div>
    ...
    <div class="mx-10 py-5 border-t border-gray-300 flex items-center">
        <div class="space-x-4 divide-x divide-gray-300 flex items-center text-xs text-gray-600">
            <div>{{ $plugins->total() }} Plugins</div>
        </div>
        <div class="ml-auto"> {{ $plugins->onEachSide(1)->links('components.ui.pagination') }} </div>
    </div>
</div>
